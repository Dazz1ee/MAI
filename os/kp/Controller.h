...
class Controller {
 ...
 private:
  ...
  void executeJob(map<int, ExecutableNode>& pid_working_jobs) {
    for (map<int, ExecutableNode>::iterator iterator_nodes = executable_nodes_.begin();
      ...
      if (iterator_nodes->second.priority == 0 && !checkExec(pid_working_jobs, iterator_nodes->second)) {
        ...
        if (process_id == 0) {
          char *buffer_argument[2 + iterator_nodes->second.node.getFiles().size()];
          buffer_argument[0] = (char *) iterator_nodes->second.node.getMutex().c_str();
          buffer_argument[1] = (char *) iterator_nodes->second.node.getType().c_str();

          int count_file = 0;

          for (; count_file < iterator_nodes->second.node.getFiles().size(); ++count_file) {
            buffer_argument[2 + count_file] =
                (char *) iterator_nodes->second.node.getFiles()[count_file].c_str();
          }

          buffer_argument[count_file+2] = NULL;
          cout << "Runnig node with id: " << iterator_nodes->first << endl;

          if(execv("child", buffer_argument) < 0){
            perror("Exec error");
            exit(-1);
          }
        }
